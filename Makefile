...
migrations-supported-features-php-ensure-no-zzz-disable-otel-attr-hooks-ini-when-opentelemetry-instrumentation-is-enabled: #### Ensure we remove `etc/qa/zzz_disable_otel_attr_hooks.ini` when opentelemetry-instrumentation is enabled ##*I*##
	($(DOCKER_RUN) php -r 'if (!in_array("opentelemetry-instrumentation", ["code-style","composer-dependency-checkers","linux","macos","static-analysis","unit-tests","windows"])) {exit;} @unlink("etc/qa/zzz_disable_otel_attr_hooks.ini");' || true)
...
help: ## Show this help ####
	@printf "%s\n" ''
	@printf "%s\n" '             ._   o   |_     _.        o   ._ _           _    /    o    _    _    ._    __   _|_   |_    ._    _            _.   |_    |    _'
	@printf "%s\n" ' \/\/   \/   |    |   | |   (_|   ><   |   | | |   |_|   _>   /     |   _>   (_)   | |         |_   | |   |    (_)   \/\/   (_|   |_)   |   (/_'
	@printf "%s\n" '        /                                                          _|'
	@printf "\n"
